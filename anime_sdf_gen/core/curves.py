...
from .messages import UserError, contour_label

import numpy as np
import json
from .model import PAD, SPAN, LTR, sweep, point
...
def handles(c, index):
    ...
    nxt = np.asarray(
        pts[(index + 1) % len(pts)]["co"] if index < len(pts) - 1 or c["closed"] else p["co"]
    )
    direction = nxt - prev
...
def _flatten(a, b, c, d, tolerance, out, depth=0):
    ...
    if length < 1e-12:
        ...
        error = (
            max(
                abs(chord[0] * (b - a)[1] - chord[1] * (b - a)[0]),
                abs(chord[0] * (c - a)[1] - chord[1] * (c - a)[0]),
            )
            / length
        )
        ...
        error = max(
            error,
            (np.linalg.norm(b - a) + np.linalg.norm(c - b) + np.linalg.norm(d - c) - length) / 4,
        )
...
def flatten(c, tolerance=0.0005):
    pts = c["points"]
...
def polygon(c, direction=LTR, tolerance=0.0005):
    arr = flatten(c, tolerance)
    if not c["closed"]:
        x = -PAD - 0.02 if direction == LTR else 1 + PAD + 0.02
...
def self_intersects(poly):
    ...
    for i in range(n):
        ...
        for j in range(i + 2, n):
            ...
            if np.any(np.maximum(a, b) < np.minimum(c, d) - 1e-10) or np.any(
                np.maximum(c, d) < np.minimum(a, b) - 1e-10
            ):
                continue
            ab, cd = b - a, d - c
            den = cross(ab, cd)
...
def fill_polygon(poly, size):
    ...
    ys = (np.arange(size, dtype=float) + 0.5) * SPAN / size - PAD
    ...
    for i in range(0, len(poly) - 1, 2):
        ...
        a = np.ceil((intersections[i, valid] + PAD) * size / SPAN - 0.5).astype(int).clip(0, size)
        b = (
            np.ceil((intersections[i + 1, valid] + PAD) * size / SPAN - 0.5)
            .astype(int)
            .clip(0, size)
        )
        diff[rows[valid], a] += 1
...
def inherited_light(project, frame, direction, size, validate=True, cache=None):
    ...
    for later in sweep(project, direction):
        if later["progress"] > frame["progress"] and any(
            c["closed"] and c["operation"] == "REMOVE" and c.get("enabled", True)
            for c in later["contours"]
        ):
