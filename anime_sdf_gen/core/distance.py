...
def squared_steps(features):
    ...
    if features.ndim != 2:
        raise ValueError("The distance transform requires a 2D mask.")
    h, w = features.shape
    infinity = float(h*h+w*w)*4
    if not features.any():
        return np.full((h, w), infinity)
    f = np.where(features, 0., infinity)
    f = yield from _rows_steps(f, "Distance: rows")
    f = yield from _rows_steps(f.T.copy(), "Distance: columns")
    return f.T.copy()
