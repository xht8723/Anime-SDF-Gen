...
from .messages import UserError, mark
...
PAD = 0.25
SPAN = 1.0 + 2.0 * PAD
DEFAULT_LANDMARKS = {"nose": [0.5, 0.28], "mouth": [0.5, 0.15], "chin": [0.5, 0.055]}
LTR, RTL = "left_to_right", "right_to_left"
SWEEP_LABELS = {LTR: mark("Left → Right"), RTL: mark("Right → Left")}
MIN_KEYFRAMES, MAX_KEYFRAMES = 2, 33
MAP_LABELS = {
    "character_left": mark("Left shadow"),
    "character_right": mark("Right shadow"),
    "face_coverage": mark("Face coverage"),
}
OUTPUT_ROUTES = ("R", "G", "B", "SEPARATE")
DEFAULT_PACKING = dict(zip(MAP_LABELS, OUTPUT_ROUTES))
BIT_DEPTHS = (8, 16, 32)
BIT_DEPTH_LABELS = {8: mark("8-bit PNG"), 16: mark("16-bit PNG"), 32: mark("32-bit Float EXR")}
ENCODING = {
    "values": "normalized switching thresholds",
...
def new_project(
    source, alignment, keyframes=9, mirror_sweeps=True, preset="CLEAN", resolution=2048, output=""
):
    ...
    return {
        ...
        "landmarks": landmarks,
        "mirror_sweeps": bool(mirror_sweeps),
        "preset": preset,
        ...
        "settings": {
            ...
            "padding": 8,
            "output": str(output),
            "packing": deepcopy(DEFAULT_PACKING),
            "bit_depth": 16,
            "smoothing": 0.0,
        },
        "authoring": {
...
def refit(project, count=None):
    ...
    directions = (LTR,) if project["mirror_sweeps"] else SWEEP_LABELS
    for direction in directions:
        ...
        for i in range(n):
            ...
            if direction == RTL:
                frame = mirror_keyframe(frame)
            frame["contours"].extend(
                deepcopy(min(old, key=lambda k: abs(k["progress"] - progress))["contours"][1:])
            )
            frames.append(frame)
        project["sweeps"][direction] = frames
    synchronize_mirror(project)
    if count is not None:
        project["authoring"]["keyframe_count"] = len(sweep(project, LTR))
    project["authoring"]["stage"] = "EDIT"


def copy_previous(project, direction, index):
    if index <= 0:
        raise UserError("The first keyframe has no previous keyframe.")
...
def insert_keyframe(project, direction, index):
    ...
    if len(frames) >= MAX_KEYFRAMES:
        raise UserError("The maximum is 33 keyframes per sweep.")
    index = max(0, min(index, len(frames) - 2))
    new = deepcopy(frames[index])
    new["id"], new["progress"] = (
        uid(),
        (frames[index]["progress"] + frames[index + 1]["progress"]) / 2,
    )
...
def remove_keyframe(project, direction, index):
    ...
    if index <= 0 or index >= len(frames) - 1:
        raise UserError("Keep the first and last keyframes of each sweep.")
    del frames[index]
    synchronize_mirror(project, direction)
    return index - 1
...
def advance_rotation(degrees, delta):
    return (float(degrees) + max(0.0, float(delta))) % 360


def validate_packing(packing):
    if not isinstance(packing, dict) or set(packing) != set(MAP_LABELS):
        raise UserError("Choose an output channel or Separate for each texture map.")
    if any(route not in OUTPUT_ROUTES for route in packing.values()):
        raise UserError("Texture maps must use R, G, B, or Separate.")
    channels = [route for route in packing.values() if route != "SEPARATE"]
    if len(channels) != len(set(channels)):
        raise UserError("Each packed RGB channel can contain only one texture map.")
    return packing
...
def set_packing(project, name, route):
    ...
    if name not in MAP_LABELS or route not in OUTPUT_ROUTES:
        raise UserError("Unknown texture map or output channel.")
...
def validate_project(p):
    if not isinstance(p, dict) or (
        p.get("format") != FORMAT_ID or p.get("format_version") != FORMAT_VERSION
    ):
        raise UserError(
    ...
    for key in ("source", "alignment", "landmarks", "sweeps", "settings", "mirror_sweeps"):
        if key not in p:
            raise UserError("Project is missing {key}.", key=key)
    if not isinstance(p["mirror_sweeps"], bool) or set(p["sweeps"]) != set(SWEEP_LABELS):
        raise UserError("The project requires two complete boundary sweeps.")
    if p["settings"]["resolution"] not in (512, 1024, 2048, 4096):
        raise UserError("Unsupported texture resolution.")
    depth = p["settings"].get("bit_depth")
    if type(depth) is not int or depth not in BIT_DEPTHS:
        raise UserError("Choose 8-bit PNG, 16-bit PNG, or 32-bit Float EXR.")
    smoothing = p["settings"].get("smoothing")
    if (
        type(smoothing) not in (float, int)
        or not math.isfinite(smoothing)
        or not 0 <= smoothing <= 8
    ):
        raise UserError("Smoothing must be a finite number from 0 to 8.")
    validate_packing(p["settings"].get("packing"))
    if p.get("encoding") != ENCODING:
        raise UserError("Unsupported texture encoding convention.")
    for direction in SWEEP_LABELS:
        frames = sweep(p, direction)
        if not MIN_KEYFRAMES <= len(frames) <= MAX_KEYFRAMES:
            raise UserError("Each sweep requires 2–33 keyframes.")
        ts = [k["progress"] for k in frames]
        if (
            any(not isinstance(t, (float, int)) or not math.isfinite(t) for t in ts)
            or ts[0] != 0
            or ts[-1] != 1
            or any(a >= b for a, b in zip(ts, ts[1:]))
        ):
            raise UserError("Keyframe positions must increase from 0 to 1 within each sweep.")
        for frame in frames:
            cs = frame["contours"]
            if not cs or cs[0]["closed"] or any(not c["closed"] for c in cs[1:]):
                raise UserError("Each keyframe needs one main boundary followed by closed shapes.")
            if len(cs) > 128:
                raise UserError("Too many shapes in a keyframe.")
            for c in cs:
                if (
                    c["operation"] not in ("ADD", "REMOVE")
                    or not (3 if c["closed"] else 2) <= len(c["points"]) <= 256
                ):
                    raise UserError("Invalid contour operation or point count.")
                for pt in c["points"]:
                    if pt["mode"] not in ("AUTO", "CORNER", "FREE"):
                        raise UserError("Invalid handle mode.")
                    for k in ("co", "left", "right"):
                        if len(pt[k]) != 2 or any(
                            not isinstance(v, (float, int)) or not math.isfinite(v) or abs(v) > 100
                            for v in pt[k]
                        ):
                            raise UserError("Invalid point or handle coordinates.")
    if p["mirror_sweeps"] and [k["progress"] for k in sweep(p, LTR)] != [
        k["progress"] for k in sweep(p, RTL)
    ]:
        raise UserError("Mirrored sweeps must have matching keyframe positions.")
    authoring = p.get("authoring", {})
    if authoring.get("stage") not in ("ORIENT", "FIT", "EDIT", "CONFIRM"):
        raise UserError("Invalid authoring stage.")
    if authoring.get("reference") not in ("NEUTRAL", "MATERIAL"):
        raise UserError("Invalid reference appearance.")
    if not isinstance(authoring.get("front_only"), bool) or not isinstance(
        authoring.get("uv_map"), str
    ):
        raise UserError("Missing or invalid authoring source settings.")
    if (
        not isinstance(authoring.get("keyframe_count"), int)
        or not MIN_KEYFRAMES <= authoring["keyframe_count"] <= 33
    ):
        raise UserError("Invalid shadow keyframe count.")
    for name, anchor in authoring.get("anchors", {}).items():
        if name not in ("nose", "mouth", "chin") or not isinstance(anchor, dict):
            raise UserError("Invalid landmark anchor.")
        if len(anchor.get("world", [])) != 3 or any(not math.isfinite(v) for v in anchor["world"]):
            raise UserError("Invalid landmark position.")
        if len(anchor.get("barycentric", [])) != 3 or any(
            not math.isfinite(v) or v < -1e-6 or v > 1 + 1e-6 for v in anchor["barycentric"]
        ):
            raise UserError("Invalid landmark barycentric coordinates.")
        if abs(sum(anchor["barycentric"]) - 1) > 1e-5 or len(anchor.get("vertices", [])) != 3:
            raise UserError("Invalid landmark surface reference.")
    return p
...
def loads(text):
    if len(text) > 16_000_000:
        raise UserError("Project file exceeds the 16 MB limit.")
