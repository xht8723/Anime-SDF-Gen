...
from .core.messages import AppError

from copy import deepcopy
import time
import bpy
from mathutils import Matrix, Quaternion, Vector
from .viewport import scale, window_region, Rect, View
...
class Editor:
    def __init__(self, session, layout=None):
        ...
        self.split_ratio = self.saved.get("split_ratio", 0.54)
    ...
    def own_screen(self, screen):
        for kind, owned in self.session.registry.ids:
            if kind == "screens":
                try:
                    if owned.as_pointer() == screen.as_pointer():
                        return
    ...
    def open(self):
        ...
        origin = self.session.origin.window
        ...
        for screen in self.workspace.screens:
            self.own_screen(screen)
    ...
    def configure(area):
        ...
        space.shading.single_color = (0.65, 0.65, 0.65)
        space.shading.show_cavity = True
        space.shading.cavity_type = "BOTH"
        space.shading.background_type = "VIEWPORT"
        space.shading.background_color = (0.055, 0.06, 0.075)
        return space
    ...
    def advance(self):
        ...
        if time.perf_counter() - self.started > 30:
            raise AppError("The floating editor could not initialize.")
        w = self.window
        if self.phase in ("OPEN", "SPLIT"):
            areas = list(w.screen.areas)
            if self.phase == "SPLIT" and len(areas) < 2:
                return
        ...
        if self.phase == "CONFIGURE":
            ...
            main.spaces.active.clip_start = max(1e-6, extent * 0.001)
    ...
    def resize(self, region):
        ...
        setup = self.session.stage in ("ORIENT", "FIT")
        confirm = self.session.stage == "CONFIRM"
        top = round(90 * u)
        shelf = 0 if setup else round((100 if confirm else 158) * u)
        panel = round(min(330 * u, w * 0.32))
        available = max(2, w - panel)
        bottom = min(shelf, h // 3)
        body_h = max(1, h - top - bottom)
        self.boxes = {
            "TOP": Rect(0, h - top, w, top),
            "SHELF": Rect(0, 0, available, bottom),
        ...
        focus = (
            "AUTHOR"
            if setup
            else "PREVIEW" if confirm else self.focus or ("AUTHOR" if available < 560 * u else None)
        )
        ...
        if focus:
            ...
            bounds = {
                "AUTHOR": Rect(0, bottom, split, body_h),
    ...
    def alive(self):
        try:
            return self.window is not None and self.window in list(
    ...
    def set_hover(self, key, position):
        ...
        moved = (
            abs(position[0] - self.hover_pos[0]) + abs(position[1] - self.hover_pos[1])
            > 3 * scale()
        )
    ...
    def tick_tooltip(self, now):
        if self.hover and not self.tooltip_ready and now - self.hover_since >= 0.6:
            self.tooltip_ready = True
            self.redraw()
    ...
    def frame(self, role=None):
        ...
        rotation = (
            Matrix((Vector(a["right"]), Vector(a["up"]), Vector(a["forward"])))
            .transposed()
            .to_quaternion()
        )
    ...
    def refresh_appearance(self):
        ...
        space.shading.type = (
            "MATERIAL" if self.session.project["authoring"]["reference"] == "MATERIAL" else "SOLID"
        )
    ...
    def snapshot(self):
        ...
        return {
            ...
            "views": {
                name: {
                    "location": list(v.view_location),
                    "rotation": list(v.view_rotation),
                    "distance": v.view_distance,
                    "perspective": v.view_perspective,
                }
                for name, v in self.views.items()
            },
        }
    ...
    def restore_views(self):
        for name, values in self.saved.get("views", {}).items():
            ...
            v.view_location = Vector(values["location"])
            v.view_rotation = Quaternion(values["rotation"])
            v.view_distance = values["distance"]
            v.update()
            v.view_perspective = values["perspective"]
            v.update()
    ...
    def free_buffers(self):
        for buffer in self.buffers.values():
            try:
                ...
                pass  # GPU context may already be gone after native X.
        self.buffers.clear()
            ...
            if len(bpy.context.window_manager.windows) > 1:
                ...
                if self._native_fullscreen:
                    with bpy.context.temp_override(window=self.window, area=self.native_area):
                        bpy.ops.screen.screen_full_area(use_hide_panels=True)
                state = self.session.origin
                with bpy.context.temp_override(
                    window=self.window, screen=state.workspace.screens[0]
                ):
                    self.window.scene = state.scene
                    self.window.view_layer = state.view_layer
