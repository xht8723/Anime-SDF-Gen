...
import bpy
from .editor import window_region

NAVIGATION = {
    'view3d.rotate','view3d.move','view3d.zoom','view3d.dolly',
    'view3d.view_axis','view3d.view_orbit','view3d.view_pan','view3d.view_roll',
    'view3d.view_persportho','view3d.view_selected','view3d.view_all',
    'view3d.ndof_orbit','view3d.ndof_orbit_zoom','view3d.ndof_pan','view3d.ndof_all',
}
LOCKED_NAVIGATION = {'view3d.move','view3d.zoom','view3d.view_pan','view3d.ndof_pan',
                     'view3d.view_selected','view3d.view_all'}
CAMERA = ('view_location','view_rotation','view_distance','view_perspective')


def camera_state(view):
    return {name:getattr(view,name).copy() if name in ('view_location','view_rotation') else getattr(view,name) for name in CAMERA}


def apply_camera(view,state):
    for name,value in state.items():setattr(view,name,value)
...
class NativeNavigation:
    ...
    def prepare(self,role,interactive=False):
        ...
        if interactive:self.host.update()
        self.editor.redraw()

    def poll(self):
        if not self.role or self._matrix_busy or not self.editor.ready:return
        slot=self.editor.views[self.role]
        before=camera_state(slot)
        apply_camera(slot,camera_state(self.host))
        slot.update()
        if before!=camera_state(slot):self.editor.redraw()

    def stop(self):
        self.poll();self.role=None
    ...
    def matrices(self,slot):
        ...
        e=self.editor
        if not e.ready or not self.drawing:return
        r=self.host;before=camera_state(r);locked=r.lock_rotation
        ...
        try:
            ...
            projection[0][0]*=(native.width/max(1,native.height))/(slot.region.width/max(1,slot.region.height))
            slot.projection_matrix=projection
            slot.perspective_matrix=projection @ slot.view_matrix
        finally:
            try:
                apply_camera(r,before);r.lock_rotation=locked
                with bpy.context.temp_override(window=e.window,area=e.native_area,region=window_region(e.native_area)):
                    r.update()
            finally:self._matrix_busy=False
