...
from .core.geometry import validate_uv, area
...
def selected_faces(obj):
    ...
    if obj.mode == "EDIT":
        ...
        return [
            ...
            and (
                f.select
                or (mode[0] and all(v.select and not v.hide for v in f.verts))
...
def capture(obj, region="SELECTED", group="", uv_name="", require_edit=True):
    if not obj or obj.type != "MESH":
        raise UserError("Make a mesh active before creating an SDF project.")
    if region == "SELECTED" and require_edit and obj.mode != "EDIT":
        raise UserError("Select facial geometry in Edit Mode, or choose Vertex Group.")
    ...
    return extract(
        obj, uv_name, region=region, selected=ids, group=group, validate=False, front_only=False
    )
...
def fit_frame(vertices, forward, up):
    ...
    if np.linalg.norm(u) < 1e-8:
        raise UserError("Face forward and up must be different directions.")
    u /= np.linalg.norm(u)
    r = np.cross(u, f)
    ...
    center += (
        r * (raw[:, 0].max() + raw[:, 0].min()) / 2 + u * (raw[:, 1].max() + raw[:, 1].min()) / 2
    )
    return {
    ...
    if obj and obj.type == "MESH" and obj.mode == "EDIT":
        ...
        try:
            ...
            for item in layer.objects:
                item.select_set(item in selection)
            layer.objects.active = active
            bpy.ops.object.mode_set(mode="EDIT")
    return _extract_object_mode(obj, *args, **kwargs)


def _extract_object_mode(
    obj,
    uv_name,
    region="SELECTED",
    selected=None,
    ...
    if not obj or obj.type != "MESH":
        raise UserError("Select a source mesh first.")
    if uv_name not in obj.data.uv_layers:
        raise UserError("The source has no UV map named {uv_name}.", uv_name=uv_name)
    if selected is None:
    ...
    if region == "SELECTED":
        ...
        raise UserError("Unknown face region type.")
    if not face_ids:
        raise UserError(
    ...
    if any(i < 0 or i >= len(obj.data.polygons) for i in face_ids):
        raise UserError("The saved face region no longer matches the mesh. Relink the source.")
    evaluated = obj.evaluated_get(bpy.context.evaluated_depsgraph_get())
    mesh = evaluated.to_mesh(
        preserve_all_data_layers=True, depsgraph=bpy.context.evaluated_depsgraph_get()
    )
    try:
        if len(mesh.vertices) != len(obj.data.vertices) or len(mesh.polygons) != len(
            obj.data.polygons
        ):
            raise UserError(
        ...
        if uv_name not in mesh.uv_layers:
            raise UserError("The evaluated mesh has lost the selected UV map.")
        mesh.calc_loop_triangles()
        tris = [t for t in mesh.loop_triangles if t.polygon_index in face_ids]
        if not tris:
            raise UserError("No face triangles were found.")
        vids = sorted({v for t in tris for v in t.vertices})
        remap = {v: i for i, v in enumerate(vids)}
        vertices = np.asarray(
            [list(obj.matrix_world @ mesh.vertices[v].co) for v in vids], dtype=np.float64
        )
        triangles = np.asarray([[remap[v] for v in t.vertices] for t in tris], dtype=np.int32)
        layer = mesh.uv_layers[uv_name]
        uvs = np.asarray(
            [[list(layer.data[li].uv) for li in t.loops] for t in tris], dtype=np.float64
        )
        all_uvs = {
            uv.name: np.asarray(
                [[list(uv.data[li].uv) for li in t.loops] for t in tris], dtype=np.float64
            )
        ...
        local[:, 0] = local[:, 0] / alignment["width"] + 0.5
        local[:, 1] = local[:, 1] / alignment["height"] + 0.5
        ...
        if front_only:
            ...
            if not len(triangles):
                raise UserError("The frontal subset is empty. Reset the face orientation.")
        if validate:
...
def from_reference(ref, alignment, obj=None, validate=True, allow_relink=False):
    ...
    face = extract(
        ...
        alignment=alignment,
        validate=validate,
        front_only=ref.get("front_only", False),
        group=ref.get("vertex_group", ""),
    )
    if not allow_relink and face.reference["faces"] != ref["faces"]:
        raise UserError("Vertex-group membership changed. Relink the source before regenerating.")
...
def world_point(alignment, xy, depth):
    return (
        Vector(alignment["center"])
        + Vector(alignment["right"]) * (xy[0] - 0.5) * alignment["width"]
        + Vector(alignment["up"]) * (xy[1] - 0.5) * alignment["height"]
        + Vector(alignment["forward"]) * depth
    )
