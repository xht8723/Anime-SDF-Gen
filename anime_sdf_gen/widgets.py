...
class Painter:
    ...
    def begin(self,width,height):
        self.rectangles.clear();self.labels.clear();self.viewport=(width,height)

    def rectangle(self,x,y,w,h,fill,radius=0,border=None,stroke=0):
        if w>0 and h>0:
            self.rectangles.append((x,y,w,h,fill,min(radius,w/2,h/2),border or fill,stroke))
    ...
    def _shader(self):
        if self.shader is None:
            ...
            for index,kind,name in [(0,'VEC2','position'),(1,'VEC2','coordinate'),
                                    (2,'VEC2','extent'),(3,'VEC2','style'),
                                    (4,'VEC4','fillColor'),(5,'VEC4','borderColor')]:
                info.vertex_in(index,kind,name)
            stage=gpu.types.GPUStageInterfaceInfo('anime_sdf_gen_widgets')
            for kind,name in [('VEC2','local'),('VEC2','halfSize'),('VEC2','shapeStyle'),
                              ('VEC4','fill'),('VEC4','edge')]:stage.smooth(kind,name)
            info.vertex_out(stage);info.push_constant('VEC2','viewportSize')
    ...
    def flush(self):
        ...
        if self.rectangles:
            attrs={name:[] for name in ('position','coordinate','extent','style','fillColor','borderColor')}
            order=(0,1,2,0,2,3)
            for item in self.rectangles:
                if item[0]=='line':
                    ...
                    dx,dy=b[0]-a[0],b[1]-a[1];length=math.hypot(dx,dy)
                    ux,uy=dx/length,dy/length;w,h=length+stroke,stroke
                    basis=((ux,uy),(-uy,ux));radius=stroke/2;edge=fill;border_width=0
                else:
                    x,y,w,h,fill,radius,edge,border_width=item
                    center=(x+w/2,y+h/2);basis=((1,0),(0,1))
                half=(w/2,h/2);pad=1.0
                corners=((-half[0]-pad,-half[1]-pad),(half[0]+pad,-half[1]-pad),
                         (half[0]+pad,half[1]+pad),(-half[0]-pad,half[1]+pad))
                for i in order:
                    x,y=corners[i]
                    attrs['position'].append((center[0]+basis[0][0]*x+basis[1][0]*y,
                                              center[1]+basis[0][1]*x+basis[1][1]*y))
                    attrs['coordinate'].append((x,y));attrs['extent'].append(half)
                    attrs['style'].append((radius,border_width));attrs['fillColor'].append(linear_color(tuple(fill)));attrs['borderColor'].append(linear_color(tuple(edge)))
            shader=self._shader();shader.bind();shader.uniform_float('viewportSize',self.viewport)
            # GPU buffers must use float32, including coordinates.
            batch_for_shader(shader,'TRIS',{k:np.asarray(v,dtype=np.float32) for k,v in attrs.items()}).draw(shader)
            self.rectangles.clear()
        from .drawing import label
        for values in self.labels:label(*values)
        self.labels.clear()

    def close(self):
        self.rectangles.clear();self.labels.clear();self.shader=None
