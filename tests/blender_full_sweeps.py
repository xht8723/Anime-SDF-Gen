...
RUN = TestRun("full-sweeps-2k").prepare(blender=True)
...
def workflow():
    ...
    for name in ("face.png", "face_right.png", "face.sdfproject.json"):
    ...
    p = model.new_project(
        face.reference, face.alignment, keyframes=8, resolution=512, output=str(OUT / "face")
    )
    p["authoring"]["stage"] = "ORIENT"
    ...
    click("KEYFRAMES_MORE")
    yield 0.15
    assert s.project["authoring"]["keyframe_count"] == 9
    click("KEYFRAMES_LESS")
    yield 0.15
    assert s.project["authoring"]["keyframe_count"] == 8
    assert action("settings", section="KEYFRAME_COUNT", keyframes=9) == {"FINISHED"}
    click("NEXT")
    yield 0.25
    assert s.stage == "FIT" and len(s.project["authoring"]["anchors"]) == 3
    click("NEXT")
    yield 0.3
    assert s.stage == "EDIT"
    redraw()
    assert (s.direction, s.key_index, s.rotation) == (model.LTR, 0, 0) and not s.playing
    assert len([w for w in e.widgets if w["key"].startswith("KEYFRAME:")]) == 18, (
        [w["key"] for w in e.widgets],
        s.project["authoring"],
    )
    ...
    yield 0.2
    shot(OUT, "01-two-complete-sweeps.png")
    hover("MIRROR")
    yield from await_tooltip("MIRROR")
    metadata()
    shot(OUT, "02-full-sweep-tooltip.png")
    report["checks"].append(
        "single fitting view, default markers, even count controls, first-frame entry, two labelled full rows and straight presets"
    )

    click("MIRROR")
    yield 0.15
    assert not s.project["mirror_sweeps"]
    primary = json.dumps(model.sweep(s.project, model.LTR), sort_keys=True)
    click("KEYFRAME:right_to_left:4")
    yield 0.15
    click("ADD_KEYFRAME")
    yield 0.15
    assert (
        len(model.sweep(s.project, model.RTL)) == 10 and len(model.sweep(s.project, model.LTR)) == 9
    )
    click("UNDO")
    yield 0.15
    assert len(model.sweep(s.project, model.RTL)) == 9
    click("REDO")
    yield 0.15
    assert len(model.sweep(s.project, model.RTL)) == 10
    click("REMOVE_KEYFRAME")
    yield 0.15
    assert json.dumps(model.sweep(s.project, model.LTR), sort_keys=True) == primary
    click("KEYFRAME:right_to_left:0")
    yield 0.15
    ...
    view = e.views["AUTHOR"]
    r = view.region
    co = s.curve["points"][3]["co"]
    xy = canvas.to_view(s, co, view)
    start = (r.x + xy.x, r.y + xy.y)
    saved = model.dumps(s.project)
    event("MOUSEMOVE", "NOTHING", start)
    event("LEFTMOUSE", xy=start)
    yield 0.1
    event("MOUSEMOVE", "NOTHING", (start[0] + 20, start[1]))
    yield 0.15
    ...
    report["checks"].append(
        "independent insertion/removal, undo/redo, protected endpoints and actual drag cancellation preserve the other row and modal controls"
    )
    ...
    assert not authored_mask()[hole].any() and not s.preview.rgba[..., 1].any()
    assert json.dumps(model.sweep(s.project, model.RTL), sort_keys=True) == opposite
    click("KEYFRAME:left_to_right:4")
    yield 0.15
    assert not authored_mask()[hole].any()
    click("KEYFRAME:left_to_right:6")
    yield 0.15
    click("LAYER:1")
    yield 0.15
    original_mask = authored_mask()
    xy = canvas.to_view(s, s.curve["points"][0]["co"], view)
    start = (r.x + xy.x, r.y + xy.y)
    event("MOUSEMOVE", "NOTHING", start)
    event("LEFTMOUSE", xy=start)
    yield 0.1
    s.frame_times = []
    for i in range(24):
        event("MOUSEMOVE", "NOTHING", (start[0] - (i + 1) * 1.2, start[1] + (i + 1) * 0.35))
        yield 0.012
    assert s.dragging and np.any(authored_mask() != original_mask)
    event("LEFTMOUSE", "RELEASE", (start[0] - 28.8, start[1] + 8.4))
    yield 0.2
    intervals = np.diff(s.frame_times) * 1000
    report["drag_frame_median_ms"] = float(np.median(intervals)) if len(intervals) else None
    click("UNDO")
    yield 0.15
    np.testing.assert_array_equal(authored_mask(), original_mask)
    click("REDO")
    yield 0.15
    assert np.any(authored_mask() != original_mask)
    click("MIRROR")
    yield 0.2
    assert s.project["mirror_sweeps"]
    yield from wait_for(lambda: s.preview.thresholds is not None or bool(s.error))
    assert not s.error, s.error
    np.testing.assert_array_equal(s.preview.thresholds[..., 0], s.preview.thresholds[:, ::-1, 1])
    np.testing.assert_array_equal(
        s.preview.thresholds, compile.compile_project(s.project, s.size, s.preview.domain)
    )
    shot(OUT, "03-local-lit-cutout.png")
    report["checks"].append(
        "lit cutouts carry only within their row, update during real dragging, survive undo/redo and mirror only into corresponding opposite frames"
    )

    # Back endpoints may differ, but they must remain representable and editable.
    click("KEYFRAME:left_to_right:8")
    yield 0.15
    click("TRIANGLE")
    yield 0.15
    action("transform", dx=-0.14)
    ...
    yield 0.15
    yield from wait_for(lambda: s.preview.thresholds is not None or bool(s.error))
    assert not s.error and s.seam_warning and "Back" in str(s.seam_warning), s.error
    redraw()
    assert e.warning_bounds.y == 0
    shot(OUT, "04-endpoint-seam-warning.png")
    click("DELETE_SHAPE")
    yield 0.15
    yield from wait_for(lambda: s.preview.thresholds is not None)
    assert not s.seam_warning
    report["checks"].append(
        "different orbit endpoints report a nonblocking footer seam warning without merging artwork"
    )

    # A complete automatic orbit must cross both hemispheres and wrap, not bounce.
    click("KEYFRAME:left_to_right:0")
    yield 0.15
    click("PLAY")
    yield 0.15
    samples = []
    deadline = time.perf_counter() + 12.5
    while time.perf_counter() < deadline:
        samples.append(s.rotation)
        yield 0.2
    ...
    for angle in (0, 90, 180, 270, 360):
        ...
        pos = (x + w * angle / 360, y + h / 2)
        if angle == 360:
            pos = (pos[0] - 0.1, pos[1])
        event("MOUSEMOVE", "NOTHING", pos)
        event("LEFTMOUSE", xy=pos)
        event("LEFTMOUSE", "RELEASE", pos)
        yield 0.15
        assert abs(s.rotation - angle) < 0.4, (s.rotation, angle)
        _, lit, shadow = appearance(flat=True)
        masks.append((int(lit.sum()), int(shadow.sum())))
    assert masks[0][1] == 0 and masks[2][0] == 0 and masks[4][1] == 0, masks
    s.flat = False
    s.sync_launcher()
    report["checks"].append(
        "12-second automatic 360-degree orbit advances through both maps and wraps; slider input decodes both hemispheres and cardinal endpoints"
    )

    click("NEXT")
    yield 0.2
    assert s.stage == "CONFIRM" and s.playing and s.rotation < 30 and "AUTHOR" not in e.boxes
    click("PACK:character_left:B")
    yield 0.15
    click("PACK:character_right:SEPARATE")
    yield 0.15
    assert s.project["settings"]["packing"] == {
        "character_left": "B",
        "character_right": "SEPARATE",
        "face_coverage": "R",
    }
    assert s.playing
    projection = s.face.projected.copy()
    rotation = list(e.views["PREVIEW"].view_rotation)
    r = e.views["PREVIEW"].region
    pos = (r.x + r.width / 2, r.y + r.height / 2)
    event("NUMPAD_6", xy=pos)
    event("NUMPAD_6", "RELEASE", pos)
    yield 0.25
    assert rotation != list(e.views["PREVIEW"].view_rotation)
    np.testing.assert_array_equal(projection, s.face.projected)
    click("FRAME")
    yield 0.15
    shot(OUT, "05-confirm-full-orbit.png")
    art = model.dumps(s.project)
    old_id = s.registry.id
    with bpy.context.temp_override(window=e.window, area=e.native_area):
        bpy.ops.wm.save_as_mainfile(
            filepath=str(OUT / "save-during-confirm.blend"), check_existing=False
        )
    yield from wait_for(
    ...
    report["checks"].append(
        "Confirm restarts a full orbit, keeps packing settings, delegates native orbit without moving projection, and resumes artwork/playback after save"
    )

    painter = e.painter
    click("GENERATE")
    yield from wait_for(lambda: session.ACTIVE is None, 90)
    assert painter.shader is None and not s.preview.field_cache and s.preview._mask_texture is None
    ...
    assert len(layout) == 2 and len(s.export.paths) == 3
    saved = files.load_project(sidecar)
    artwork = dict(saved)
    artwork.pop("textures", None)
    assert model.dumps(artwork) == model.dumps(s.project)
    for entry in layout:
        codes = read_png(entry["path"])
    ...
    assert before == {k: len(getattr(bpy.data, k)) for k in before}
    report["checks"].append(
        "mixed RGB/grayscale export and project reopening preserve new schema, artwork and map routes; no source or datablock changes after finish/reopen/cancel"
    )
    anime_sdf_gen.unregister()


run_ui(workflow, RUN, "full-sweeps.json", timeout=240)
