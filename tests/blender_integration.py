...
def original_digest():
    ...
    for uv in mesh.uv_layers:
        vals = np.empty(len(mesh.loops) * 2, dtype=np.float32)
...
def create():
    ...
    project = model.new_project(
        face.reference, face.alignment, resolution=512, output=str(OUT / "face_sdf")
    )
...
def run_all():
    ...
    for _ in range(3):
        ...
        assert s.preview.rgba.dtype == np.float32
        session.end_session(True)
        assert_clean(before)
    result["tests"].append(
        "three independent preview/cancel cycles restore all original data and counts"
    )
    orphan = bpy.data.images.new("Unrelated orphan", width=2, height=2)
    with_orphan = original_digest()
    create()
    session.end_session(True)
    assert_clean(with_orphan)
    ...
    with patch.object(
        session.Session,
        "write_draft",
        side_effect=OSError("injected unavailable recovery directory"),
    ):
        create()
    ...
    assert_clean(before)
    result["tests"].append(
        "recovery IO failure does not prevent cleanup, save suspension, or resume"
    )
    s = create()
    ...
    bpy.ops.anime_sdf_gen.transform(dx=0.02, dy=0.01, angle=12, scale=1.1)
    ...
    assert len(s.curve["points"]) == 4
    s.project = model.loads(original)
    s.changed()
    ...
    with bpy.context.temp_override(scene=s.origin.scene, view_layer=s.origin.view_layer):
        obj = bpy.data.objects["body"]
    ...
    s.export.start()
    while s.busy:
        s.export.advance(0.05, finish=False)
    ...
    s.export.start()
    s.export.advance(0.001, finish=False)
    s.export.cancel()
    ...
    try:
        ...
        np.testing.assert_allclose(
            pixels.reshape(512, 512, 4)[..., :3], png.astype(float) / 65535, atol=1e-6
        )
    finally:
        bpy.data.images.remove(img)
    result["tests"].append(
        "Blender reload preserves 16-bit channel values without color conversion"
    )
    ...
    assert session.ACTIVE is None and session.SUSPENDED
    assert not any(OWNER_KEY in sc for sc in bpy.data.scenes)
    session.resume()
    assert session.ACTIVE is not None
    ...
    bpy.ops.wm.open_mainfile(filepath=str(RUN.fixture), load_ui=False, use_scripts=False)
    assert session.ACTIVE is None
    assert_clean(before)
    ...
    assert fixture_hash == hashlib.sha256(RUN.fixture.read_bytes()).hexdigest()
