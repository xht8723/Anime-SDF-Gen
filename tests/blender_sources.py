...
OUT=ROOT/'build'/'validation'/'v014';OUT.mkdir(parents=True,exist_ok=True)
...
def failure(call,fragment):
    ...
    else:raise AssertionError('Expected '+fragment)


try:
    mesh=bpy.data.meshes.new('Arbitrary topology')
    verts=[(x,0,z) for z in range(3) for x in range(3)]
    faces=[(x+3*z,x+1+3*z,x+4+3*z,x+3+3*z) for z in range(2) for x in range(2)]
    mesh.from_pydata(verts,[],faces);mesh.update()
    uv=mesh.uv_layers.new(name='Character Atlas')
    for p in mesh.polygons:
        for li in p.loop_indices:
            co=mesh.vertices[mesh.loops[li].vertex_index].co;uv.data[li].uv=(co.x/2,co.z/2)
    obj=bpy.data.objects.new('Any Mesh Name',mesh);bpy.context.scene.collection.objects.link(obj)
    ...
    group.add(list(faces[0]),1,'REPLACE');group.add([2,5],0,'REPLACE')
    ...
    report['checks'].append('arbitrary mesh/group/UV names; all vertices need strictly positive group weights')
    group.add([2,5],.0001,'REPLACE')
    ...
    for mode in ((True,False,False),(False,True,False),(False,False,True)):
        bpy.context.tool_settings.mesh_select_mode=mode;bpy.ops.object.mode_set(mode='EDIT')
        ...
        if mode[0]:
            for vertex in bm.faces[0].verts:vertex.select_set(True)
        elif mode[1]:
            for edge in bm.faces[0].edges:edge.select_set(True)
        else:bm.faces[0].select_set(True)
        bm.select_flush_mode();bmesh.update_edit_mesh(mesh)
    ...
    report['checks'].append('vertex/edge/face selections, Edit Mode restoration, and immutable captured polygon IDs')
    bpy.ops.object.mode_set(mode='EDIT');bm=bmesh.from_edit_mesh(mesh);bm.faces.ensure_lookup_table()
    bm.faces[0].select_set(True);bm.faces[0].hide_set(True);bmesh.update_edit_mesh(mesh)
    failure(lambda:source.capture(obj),'empty');bpy.ops.object.mode_set(mode='OBJECT')
    report['checks'].append('hidden selected geometry excluded')
    mesh2=mesh.copy();obj2=bpy.data.objects.new('Another Mesh',mesh2);bpy.context.scene.collection.objects.link(obj2)
    obj2.select_set(True);obj.select_set(True);bpy.context.view_layer.objects.active=obj
    bpy.ops.object.mode_set(mode='EDIT')
    for target in (obj,obj2):
        bm=bmesh.from_edit_mesh(target.data)
        for face in bm.faces:face.hide_set(False);face.select_set(True)
    ...
    report['checks'].append('multi-object Edit Mode captures only active mesh and preserves both edit states')
    bpy.ops.object.mode_set(mode='OBJECT')
    uv_name=mesh.uv_layers.active.name;mesh.uv_layers.remove(mesh.uv_layers.active)
    failure(lambda:source.capture(obj,'VERTEX_GROUP',group.name),'UV map')
    report['checks'].append('missing UVs rejected')
    report['status']='PASS'
except Exception:
    report['status']='FAIL';report['traceback']=traceback.format_exc();traceback.print_exc()
