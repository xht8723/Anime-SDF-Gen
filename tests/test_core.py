from tests.samples import project, ellipse
...
import numpy as np
from anime_sdf_gen.core import model, curves, distance, geometry, files
...
class DistanceTests(unittest.TestCase):
    def test_exact_against_brute_force(self):
        ...
        for shape in ((3, 7), (13, 9), (24, 24)):
            ...
            expected = np.min(
                (yy[..., None] - seeds[:, 0]) ** 2 + (xx[..., None] - seeds[:, 1]) ** 2, axis=-1
            )
            np.testing.assert_array_equal(run(distance.squared_steps(mask)), expected)
...
class CurveTests(unittest.TestCase):
    def test_smooth_triangle(self):
        ...
        for i, p in enumerate(c["points"]):
            left, right = curves.handles(c, i)
            a = np.asarray(p["co"]) - left
            b = right - p["co"]
            self.assertGreater(np.linalg.norm(a), 0)
            self.assertAlmostEqual(np.dot(a, b) / (np.linalg.norm(a) * np.linalg.norm(b)), 1.0)
        sharp = copy.deepcopy(c)
        for point in sharp["points"]:
            point["mode"] = "CORNER"
        self.assertGreater(
            np.count_nonzero(smooth != curves.fill_polygon(curves.polygon(sharp), 256)), 10
        )

    def test_ellipse_fill_hole_and_order(self):
        p = project()
        p["sweeps"][model.LTR][0]["contours"][0]["enabled"] = False
        p["sweeps"][model.LTR][0]["contours"] += [
            ellipse("outer", 0.5, 0.5, 0.3, 0.3),
            ellipse("hole", 0.5, 0.5, 0.1, 0.1, "REMOVE"),
        ]
        mask = curves.raster_keyframe(p, p["sweeps"][model.LTR][0], model.LTR, 128)
        self.assertFalse(mask[64, 64])
        self.assertTrue(mask[64, 80])
        self.assertFalse(mask[10, 10])
        p["sweeps"][model.LTR][0]["contours"].append(ellipse("refill", 0.5, 0.5, 0.04, 0.04))
        self.assertTrue(
            curves.raster_keyframe(p, p["sweeps"][model.LTR][0], model.LTR, 128)[64, 64]
        )

    def test_lit_triangle_inside_shadow_and_across_boundary(self):
        p = project(9, mirror=False)
        phase = p["sweeps"][model.LTR][-1]
        size = 256
        uncut = curves.raster_keyframe(p, phase, model.LTR, size)
        for dx in (-0.20, -0.03):
            shape = model.triangle()
            shape["operation"] = "REMOVE"
            editing.transform_contour(shape, dx=dx)
            phase["contours"] = phase["contours"][:1] + [shape]
            cutout = curves.fill_polygon(
                curves.polygon(shape, tolerance=model.SPAN / (size * 4)), size
            )
            ...
            refill = copy.deepcopy(shape)
            refill["operation"] = "ADD"
    ...
    def test_split_preserves_fill(self):
        c = ellipse("test", 0.5, 0.5, 0.3, 0.2)
...
class SchemaTests(unittest.TestCase):
    def test_current_schema_only(self):
        ...
        for change in (
            {"format": "different_tool"},
            {"format_version": 1},
            {"format_version": 2},
            {"format_version": model.FORMAT_VERSION + 1},
        ):
        ...
        with self.assertRaises(ValueError):
            model.loads(json.dumps(invalid))
        invalid = copy.deepcopy(p)
        invalid["authoring"].pop("front_only")
        with self.assertRaisesRegex(ValueError, "authoring source settings"):
            model.loads(json.dumps(invalid))
    ...
    def test_surface_anchor_roundtrip_and_refit_retains_patches(self):
        ...
        p["authoring"].update(
            stage="FIT",
            anchors={
                "nose": {"world": [1, 2, 3], "vertices": [5, 8, 11], "barycentric": [0.2, 0.3, 0.5]}
            },
        )
        self.assertEqual(p, model.loads(model.dumps(p)))
        patch_ = model.triangle()
        p["sweeps"][model.LTR][0]["contours"].append(patch_)
        p["landmarks"]["nose"][1] = 0.4
        model.refit(p)
        self.assertEqual(p["sweeps"][model.LTR][0]["contours"][1], patch_)
        self.assertEqual(p["authoring"]["stage"], "EDIT")
        bad = copy.deepcopy(p)
        bad["authoring"]["anchors"]["nose"]["barycentric"] = [1, 1, 1]
        with self.assertRaises(ValueError):
            model.validate_project(bad)
...
class UVTests(unittest.TestCase):
    ...
    def test_uv_transfer_padding_and_orientation(self):
        uv = np.array([[[0.1, 0.1], [0.9, 0.1], [0.9, 0.9]], [[0.1, 0.1], [0.9, 0.9], [0.1, 0.9]]])
        projected = np.concatenate([(uv - 0.1) / 0.8, np.zeros((2, 3, 1))], axis=-1)
        depth, facing = run(geometry.projection_steps(projected, 64))
        t = np.zeros((64, 64, 2), np.float32)
        t[..., 0] = 0.25
        t[..., 1] = 0.75
        image = run(geometry.bake_steps(t, uv, projected, depth, facing, 64))
        cov = image[..., 2].copy()
        np.testing.assert_allclose(image[32, 32], [0.25, 0.75, 1])
        run(geometry.pad_steps(image, 3))
        np.testing.assert_array_equal(cov, image[..., 2])
        np.testing.assert_allclose(image[32, 4, :2], [0.25, 0.75])

    def test_occluded_surface_coverage_and_scale(self):
        uv = np.array(
            [[[0.05, 0.05], [0.45, 0.05], [0.05, 0.45]], [[0.55, 0.55], [0.95, 0.55], [0.55, 0.95]]]
        )
        front = np.array([[0, 0, 0.2], [1, 0, 0.2], [0, 1, 0.2]])
        projected = np.stack([front, front - [0, 0, 0.3]])
        for scale in (1e-5, 1, 1e5):
            geometry.validate_uv(uv, projected * scale)
        depth, facing = run(geometry.projection_steps(projected, 64))
        t = np.full((64, 64, 2), 0.4, dtype=np.float32)
        image = run(geometry.bake_steps(t, uv, projected, depth, facing, 64))
...
class FileTests(unittest.TestCase):
    def test_png16_roundtrip_and_atomic_export(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            oldtex = Path(texture).read_bytes()
            oldjson = Path(sidecar).read_bytes()
            ...
            with patch.object(files.os, "replace", side_effect=fail_commit):
                with self.assertRaises(OSError):
                    files.export_bundle(p, 1 - image, Path(tmp) / "face")
