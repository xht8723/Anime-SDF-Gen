...
class SelectionTests(unittest.TestCase):
    def test_box_directions_boundary_and_hidden_points(self):
        ...
        frame=artwork();frame['contours'][1]['enabled']=False
...
class TransformTests(unittest.TestCase):
    def test_move_only_selected_points_preserves_relative_handles(self):
        frame=artwork();original=copy.deepcopy(frame)
        t=editing.Transform(frame,{(0,3),(0,4)},'MOVE',[0,0],aspect=2)
        t.move([.12,-.03]);result=t.evaluate()
        for ci,c in enumerate(frame['contours']):
            for pi,p in enumerate(c['points']):
                if (ci,pi) in t.selected:
                    np.testing.assert_allclose(result['contours'][ci]['points'][pi]['co'],np.asarray(p['co'])+[.12,-.03])
                    self.assertEqual(result['contours'][ci]['points'][pi]['left'],p['left'])
                else:self.assertEqual(result['contours'][ci]['points'][pi],p)
    ...
    def test_rotation_in_physical_face_plane_preserves_lengths(self):
        frame=artwork();sel={(1,i) for i in range(3)}
        t=editing.Transform(frame,sel,'ROTATE',[.7,.3],aspect=2.3);t.number='90'
        result=t.evaluate();before=np.asarray([p['co'] for p in frame['contours'][1]['points']])
        after=np.asarray([p['co'] for p in result['contours'][1]['points']])
        np.testing.assert_allclose(after.mean(axis=0),before.mean(axis=0))
        old=(before-before.mean(axis=0))*t.metric;new=(after-after.mean(axis=0))*t.metric
    ...
    def test_constrained_scale_transforms_free_handles(self):
        frame=artwork()
        for p in frame['contours'][1]['points']:p['mode']='FREE'
        t=editing.Transform(frame,{(1,i) for i in range(3)},'SCALE',[.7,.3]);t.axis='X';t.number='-2'
        result=t.evaluate()
        ...
        t.axis='Z';self.assertEqual(t.evaluate(),frame)

    def test_free_handle_edit_keeps_anchor_and_other_handle(self):
        frame=artwork();p=frame['contours'][1]['points'][0]
        ...
        t.axis='Y';t.number='.025';result=t.evaluate()['contours'][1]['points'][0]
        self.assertEqual(result['co'],p['co']);self.assertEqual(result['mode'],'FREE')
        np.testing.assert_allclose(result['left'],left-p['co'])
        np.testing.assert_allclose(result['right'],right-p['co']+[0,.025])

    def test_numeric_input_backspace_axis_and_identity(self):
        frame=artwork();t=editing.Transform(frame,{(0,3)},'MOVE',[0,0])
        for char in '-.125':t.numeric(char)
        t.numeric('BACK_SPACE');self.assertEqual(t.number,'-.12')
        t.axis='X';t.move([.4,.6])
        np.testing.assert_allclose(t.values(True)[0],[-.12,0])
        t.axis='Z';self.assertEqual(t.evaluate(),frame)
        t.axis='Y';np.testing.assert_allclose(t.values()[0],[0,-.12])

    def test_precision_is_incremental_and_snap_respects_numeric_entry(self):
        t=editing.Transform(artwork(),{(0,3)},'MOVE',[0,0])
        t.move([.02,0]);t.move([.12,0],True);t.move([.13,0],False)
        np.testing.assert_allclose(t.values()[0],[.04,0])
        t.move([.1437,0]);np.testing.assert_allclose(t.values(True)[0],[.05,0])
        np.testing.assert_allclose(t.values(True,True)[0],[.054,0])
        t.axis='X';t.number='.0123';np.testing.assert_allclose(t.values(True)[0],[.0123,0])

    def test_mouse_rotation_wrap_and_scale_snap(self):
        f=artwork();sel={(1,i) for i in range(3)};center=np.mean([p['co'] for p in f['contours'][1]['points']],axis=0)
        t=editing.Transform(f,sel,'ROTATE',center+[1,0])
        for angle in (170,190,350,370):t.move(center+[math.cos(math.radians(angle)),math.sin(math.radians(angle))])
        self.assertAlmostEqual(math.degrees(t.values()[1]),370)
        t=editing.Transform(f,sel,'SCALE',center+[.1,0]);t.move(center+[.137,0])
        self.assertAlmostEqual(t.values(True)[2],1.4)

    def test_invalid_values_do_not_mutate_original(self):
        frame=artwork();original=copy.deepcopy(frame)
        with self.assertRaises(ValueError):editing.Transform(frame,set(),'MOVE',[0,0])
        t=editing.Transform(frame,{(0,3)},'MOVE',[0,0]);t.axis='X';t.number='101'
        with self.assertRaises(ValueError):t.evaluate()
        self.assertEqual(frame,original)
