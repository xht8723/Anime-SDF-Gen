...
def triangle(dx=0):
    c = model.triangle()
    c["operation"] = "REMOVE"
    editing.transform_contour(c, dx=dx)
    return c
...
def all_shadow(p):
    ...
    for ph in p["sweeps"][model.RTL]:
        for pt in ph["contours"][0]["points"]:
            pt["co"][0] = -0.23
...
class LitCutoutTests(unittest.TestCase):
    def test_reported_cutout_carries_back_and_encodes_without_conflict(self):
        ...
        c = triangle(-0.14)
        p["sweeps"][model.LTR][-1]["contours"].append(c)
        model.synchronize_mirror(p)
        saved = copy.deepcopy(p)
        hole = footprint(c, size)
        ...
        for i, ph in enumerate(model.sweep(p, model.LTR)):
            ...
            np.testing.assert_array_equal(
                decode(thresholds[..., 0], ph["progress"])[domain], expected[domain]
            )
        ...
        self.assertFalse(decode(codes[..., 0].astype(float) / 65535, 1)[hole].any())
    ...
    def test_moved_hidden_and_deleted_cutout_invalidates_earlier_distance_fields(self):
        ...
        def cached():
            return run(compile.compile_steps(p, size, field_cache=fields, mask_cache=masks))

        initial = cached()
        old_hole = footprint(c, size)
        editing.transform_contour(c, dx=-0.15)
        moved = cached()
        new_hole = footprint(c, size)
        np.testing.assert_array_equal(moved, compile.compile_project(p, size))
        self.assertGreater(np.max(np.abs(moved - initial)), 0.1)
        vacated = old_hole & ~new_hole
        self.assertTrue(np.any(decode(moved[..., 0], 1)[vacated]))
        c["enabled"] = False
        np.testing.assert_array_equal(cached(), baseline)
        c["enabled"] = True
        cached()
        p["sweeps"][model.LTR][-1]["contours"].pop()
        np.testing.assert_array_equal(cached(), baseline)

    def test_first_frames_are_independent_and_carry_is_sweep_local(self):
        p = project(5, mirror=False)
        size = 128
        all_shadow(p)
        a, b = triangle(-0.22), triangle(0.22)
        ...
        for channel, direction, hole, other in ((0, model.LTR, ha, hb), (1, model.RTL, hb, ha)):
            for frame in model.sweep(p, direction):
                ...
                self.assertFalse(mask[hole].any())
                self.assertTrue(mask[other].all())
                np.testing.assert_array_equal(
                    decode(thresholds[..., channel], frame["progress"])[domain], mask[domain]
                )
        seams = endpoint_seams(thresholds, domain)
        self.assertGreater(seams["Front"], 0)
    ...
    def test_layer_order_only_carries_surviving_light(self):
        ...
        all_shadow(p)
        c = triangle(-0.1)
        refill = ellipse("Refill", 0.4, 0.3, 0.025, 0.018, "ADD")
        p["sweeps"][model.LTR][-1]["contours"] += [c, refill]
        hole = footprint(c, size)
        filled = footprint(refill, size)
    ...
    def test_moving_and_disappearing_cutouts_remain_monotonic(self):
        ...
        size = 128
        all_shadow(p)
        a, b = triangle(-0.20), triangle(0.15)
        p["sweeps"][model.LTR][1]["contours"].append(a)
        p["sweeps"][model.LTR][2]["contours"].append(b)
        ha, hb = footprint(a, size), footprint(b, size)
    ...
    def test_invalid_future_curve_cannot_reuse_unvalidated_mask(self):
        ...
        with self.assertRaisesRegex(ValueError, "crosses itself|coincident"):
            run(compile.compile_steps(p, 64, mask_cache=cache))

    def test_unrelated_shadow_reversal_still_rejected(self):
        p = project(9)
        c = triangle(-0.14)
        p["sweeps"][model.LTR][-1]["contours"].append(c)
        p["sweeps"][model.LTR][0]["contours"].append(
            ellipse("Shadow patch", 0.8, 0.7, 0.04, 0.04, "ADD")
        )
        with self.assertRaises(compile.KeyframeConflict):
            compile.compile_project(p, 128)
