...
class StraightPresetTests(unittest.TestCase):
    def test_clean_boundaries_stay_straight_after_fitting_and_mirroring(self):
        for mirror in (False, True):
            p = project(9, mirror)
            p['landmarks'] = {'nose': [.68, .39], 'mouth': [.40, .19], 'chin': [.57, .06]}
            for count in (9, 17):
                model.refit(p, count)
                for side in (model.LTR, model.RTL):
                    for ph in model.sweep(p, side):
                        ...
                        if ph['progress']:
                            expected = -.2 + ph['progress']*1.4
                            self.assertAlmostEqual(x, expected if side == model.LTR else 1-expected)
                self.assertEqual(model.loads(model.dumps(p)), p)

    def test_nose_accent_keeps_fitted_profile(self):
        p = project(); p['preset'] = 'NOSE'
        p['landmarks']['nose'][0] = .61
        model.refit(p)
        c = p['sweeps'][model.LTR][len(p['sweeps'][model.LTR])//2]['contours'][0]
        self.assertGreater(np.ptp(curves.flatten(c)[:, 0]), .1)
        self.assertAlmostEqual(c['points'][3]['co'][0], .72)
...
class PackingTests(unittest.TestCase):
    ...
    def test_every_valid_packing_roundtrips_exact_values_and_map_metadata(self):
        image = self.image(); canonical = files.quantize(image)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            for choices in itertools.product(model.OUTPUT_ROUTES, repeat=3):
                ...
                if len(rgb) != len(set(rgb)): continue
                count += 1
                p = project(); p['settings']['packing'] = dict(zip(model.MAP_LABELS, choices))
                ...
                for path, info in zip(paths, saved['textures']):
                    data = Path(path).read_bytes(); codes = files.read_png16(path)
                    ...
                    for channel, name in info['channels'].items():
                        c = list(model.MAP_LABELS).index(name); seen.append(name)
    ...
    def test_routing_swaps_occupied_channels_and_allows_multiple_individuals(self):
        ...
        self.assertEqual(p['settings']['packing'], dict(character_left='B', character_right='G', face_coverage='R'))
        model.set_packing(p, 'character_left', 'SEPARATE')
        model.set_packing(p, 'character_left', 'G')
        self.assertEqual(p['settings']['packing'], dict(character_left='G', character_right='SEPARATE', face_coverage='R'))
        for name in model.MAP_LABELS: model.set_packing(p, name, 'SEPARATE')
        self.assertEqual([v.name for v in files.output_paths(p, 'face')],
                         ['face_left.png', 'face_right.png', 'face_coverage.png', 'face.sdfproject.json'])
        self.assertEqual(model.loads(model.dumps(p)), p)
        before = copy.deepcopy(p)
        with self.assertRaises(ValueError): model.set_packing(p, 'missing', 'R')
        with self.assertRaises(ValueError): model.set_packing(p, 'character_left', 'A')
        self.assertEqual(p, before)

    def test_missing_duplicate_and_unknown_routes_rejected(self):
        for packing in (None, {}, {'character_left': 'R'},
                        dict(character_left='R', character_right='R', face_coverage='B'),
                        dict(character_left='A', character_right='G', face_coverage='B')):
            p = project(); p['settings']['packing'] = packing
            with self.assertRaises(ValueError): model.dumps(p)
        p = project(); p['encoding']['colorspace'] = 'sRGB'
        with self.assertRaisesRegex(ValueError, 'encoding'): model.dumps(p)


class BundleFailureTests(unittest.TestCase):
    def setUp(self):
        self.p = project()
        for name in model.MAP_LABELS: model.set_packing(self.p, name, 'SEPARATE')
    ...
    def test_rollback_at_each_backup_and_commit_keeps_existing_outputs(self):
        for existing_count in (0, 2, 4):
            for fail_at in range(existing_count+4):
                with self.subTest(existing=existing_count, fail_at=fail_at), tempfile.TemporaryDirectory() as tmp:
                    base = Path(tmp)/'face'; paths = files.output_paths(self.p, base)
                    original = {path.name: f'original {i}'.encode() for i, path in enumerate(paths[:existing_count])}
                    for name, data in original.items(): (Path(tmp)/name).write_bytes(data)
                    replace = files.os.replace; calls = 0
                    def fail_once(src, dst):
                        nonlocal calls
                        calls += 1
                        if calls == fail_at+1: raise OSError('injected replacement failure')
                        return replace(src, dst)
                    with patch.object(files.os, 'replace', side_effect=fail_once):
                        with self.assertRaises(OSError): files.export_bundle(self.p, self.image, base)
    ...
    def test_cancel_after_each_staging_yield_keeps_outputs_and_removes_temporary_files(self):
        for stage_count in range(1, 5):
            with tempfile.TemporaryDirectory() as tmp:
                ...
                for _ in range(stage_count): next(steps)
                steps.close()
                self.assertEqual({p.name: p.read_bytes() for p in Path(tmp).iterdir()}, original)
    ...
    def test_bad_verification_and_folder_collision_cannot_replace_outputs(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            paths = files.export_bundle(self.p, self.image, base)
            original = {Path(p).name: Path(p).read_bytes() for p in paths}
            with patch.object(files, 'read_png16', return_value=np.zeros((1, 1), np.uint16)):
