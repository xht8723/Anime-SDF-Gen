...
class SweepTests(unittest.TestCase):
        ...
        for mirrored in (False,True):
            ...
            if not mirrored:
                for k in model.sweep(p,model.RTL):
                    for pt in k['contours'][0]['points']:pt['co'][0]-=.06*math.sin(k['progress']*math.pi)
                model.insert_keyframe(p,model.RTL,2)
            before=copy.deepcopy(p);t=compile.compile_project(p,128);domain=compile.face_domain(128)
            ...
            for channel,direction in enumerate(model.SWEEP_LABELS):
                for k in model.sweep(p,direction):
                    ...
                    np.testing.assert_array_equal(compile.decode(t[...,channel],k['progress'])[domain],expected[domain])
            if mirrored:np.testing.assert_allclose(t[...,0],t[:,::-1,1],atol=1e-6)
            else:self.assertGreater(np.max(abs(t[...,0]-t[:,::-1,1])),.01)

    def test_full_sweep_mirror_does_not_pair_frames_inside_one_row(self):
        p=project(8);before=copy.deepcopy(model.sweep(p,model.LTR));index=3
        frame=model.get_keyframe(p,model.LTR,index)
        frame['contours'][0]['points'][3]['co'][0]+=.02
        model.synchronize_mirror(p,model.LTR)
        for i,k in enumerate(model.sweep(p,model.LTR)):
            if i!=index:self.assertEqual(k,before[i])
    ...
    def test_independent_insert_remove_copy_and_history(self):
        p=project(6,False);other=copy.deepcopy(model.sweep(p,model.RTL));h=model.History();h.push(p)
        i=model.insert_keyframe(p,model.LTR,1)
        self.assertEqual(i,2);self.assertEqual(len(model.sweep(p,model.LTR)),7)
        self.assertEqual(model.sweep(p,model.RTL),other)
        model.copy_previous(p,model.LTR,i)
        p=h.undo(p);self.assertEqual(len(model.sweep(p,model.LTR)),6)
        p=h.redo(p);model.remove_keyframe(p,model.LTR,i)
        self.assertEqual(len(model.sweep(p,model.LTR)),6)
        for i in (0,5):
            with self.assertRaises(ValueError):model.remove_keyframe(p,model.LTR,i)
        p['mirror_sweeps']=True;model.synchronize_mirror(p,model.RTL)
    ...
    def test_conflict_labels_identify_row_and_one_based_frames(self):
        p=project(6,False)
        for pt in model.get_keyframe(p,model.RTL,3)['contours'][0]['points']:pt['co'][0]=1.1
        with self.assertRaises(compile.KeyframeConflict) as caught:compile.compile_project(p,64)
...
class RotationTests(unittest.TestCase):
    def test_cardinals_wrap_and_long_frames(self):
        for angle,expected in ((0,(model.LTR,0)),(90,(model.LTR,.5)),(180,(model.LTR,1)),
                               (270,(model.RTL,.5)),(360,(model.LTR,0)),(-90,(model.RTL,.5)),(810,(model.LTR,.5))):
        ...
        for direction in model.SWEEP_LABELS:
            for p in (.01,.25,.5,.99):
                self.assertEqual(model.rotation_sample(model.keyframe_rotation(direction,p))[0],direction)
                self.assertAlmostEqual(model.rotation_sample(model.keyframe_rotation(direction,p))[1],p)

    def test_one_orbit_visits_both_maps_and_full_face_not_half_face(self):
        p=project(9);t=compile.compile_project(p,128);domain=compile.face_domain(128)
        masks=[]
        for angle in np.linspace(0,360,721):
            direction,progress=model.rotation_sample(angle)
            masks.append(compile.decode(t[...,0 if direction==model.LTR else 1],progress))
        np.testing.assert_array_equal(masks[0],masks[-1])
        self.assertFalse(masks[0][domain].any());self.assertTrue(masks[360][domain].all())
        self.assertGreater(masks[270][domain].mean(),.75)
        for i in range(360):self.assertFalse((masks[i]&~masks[i+1]&domain).any())
        for i in range(360,720):self.assertFalse((~masks[i]&masks[i+1]&domain).any())
        np.testing.assert_array_equal(masks[180][:,::-1],masks[540])
        self.assertEqual(compile.endpoint_seams(t,domain),{'Front':0,'Back':0})

    def test_endpoint_warning_detects_art_difference_without_mutation(self):
        p=project(5,False);cut=model.triangle();cut['operation']='REMOVE'
        model.get_keyframe(p,model.LTR,-1)['contours'].append(cut)
        before=copy.deepcopy(p);t=compile.compile_project(p,64)
        seams=compile.endpoint_seams(t,compile.face_domain(64))
        self.assertEqual(seams['Front'],0);self.assertGreater(seams['Back'],0)
        self.assertEqual(p,before)
